...
def Ucart(request):
    ...
    amounts = sum(int(i.price.replace('₹','')) for i in ad)
    return render(request,'user/Ucart.html', {'ad': ad, 'sum': amounts})
...
def Product_payment(request):
    ...
    payment_order = client.order.create(dict(amount=amt, currency="INR", payment_capture=1))
    payment_order_id = payment_order['id']
    product_booking.objects.filter(user=request.session['user_id']).update(payment_status=True)
    return render(request, 'user/Product_payment.html', {'a': amounts, 'api_key': api_key, 'order_id': payment_order_id})

# def Product_payment(request):
#     api_key = RAZORPAY_API_KEY

#     cart_items = product_booking.objects.filter(
#         user=request.session['user_id'],
#         payment_status=False
#     )

#     amount = sum(
#         int(i.price.replace('₹', '').strip())
#         for i in cart_items
#         if i.price and i.price.replace('₹', '').strip().isdigit()
#     )

#     # if amount < 1:
#     #     return HttpResponse("Invalid cart amount")

#     amt = amount * 100  # paise
...
#     return render(request, 'user/Product_payment.html', {
#         'a': amount,
#         'api_key': api_key,
